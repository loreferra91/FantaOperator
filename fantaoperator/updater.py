...
from .gazzetta_votes import is_gazzetta_votes_url, gazzetta_votes_url, parse_gazzetta_votes, PROVIDER, EDITION
...
def refresh_votes(db, league, matchday):
    ...
    with collector_lock(db):
        try:
            ...
            if is_gazzetta_votes_url(url):
                if league["vote_provider"] != PROVIDER or league["vote_edition"] != EDITION:
                    raise ValueError("Configura provider Gazzetta e redazione La Gazzetta dello Sport per questo URL.")
                if not url.startswith(gazzetta_votes_url(league["season"])):
                    raise ValueError("La stagione nell'URL Gazzetta non coincide con la lega: aggiorna l'URL.")
                requested_url = gazzetta_votes_url(league["season"], matchday)
                payload, mime, final_url = fetch_url(requested_url, allow_html=True)
                if final_url.rstrip("/") != requested_url:
                    raise ValueError("Redirect della pagina Gazzetta: giornata non disponibile o accesso da verificare.")
                batch = parse_gazzetta_votes(payload, source_url=final_url, provider=league["vote_provider"],
                    edition=league["vote_edition"], season=league["season"], matchday=matchday)
                result = db.import_records(league["id"], matchday, batch.records,
